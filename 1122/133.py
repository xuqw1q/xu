...
class FFPlayer:
    ...
    def perform_seek_improved(self, target_pos, was_playing):
        ...
        try:
            ...
            with self.player_lock:
                ...
                if self.player:
                    try:
                        ...
                        # 简化的seek验证 - 只等待1帧
                        self.wait_for_seek_simple(target_pos)

                    except Exception as e:
                        # 只有在seek失败时才重新创建播放器
                        print(f"Direct seek failed, recreating player: {e}")
                        try:
                            self.player.close_player()
                        except:
                            pass
            ...
            self.root.after(100, release_slider_lock)  # 从300ms减少到100ms

    def wait_for_seek_simple(self, target_pos, max_attempts=10):
        """简化的seek等待 - 减少延迟"""
        for attempt in range(max_attempts):
            try:
                ...
                time.sleep(0.02)  # 减少等待时间从50ms到20ms
